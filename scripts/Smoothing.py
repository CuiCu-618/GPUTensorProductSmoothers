...
for i in range(3, 11):
    files_d.append(f'/scratch/cucui/GPUTensorProductSmoothers/build_DG/TESTING/poisson_2D_DGQ{i}_ConflictFree_ConflictFree_GLOBAL_ConflictFree_ExactRes_multiple_double.log')
data_d = read_data(files_d)

scale = 1e9

data_0 = extract_component(data_d, "S", 3, 0)
data_0_L = np.array([data_p[-1] for data_p in data_0]) / scale

data_1 = extract_component(data_d, "S", 3, 1)
data_1_L = np.array([data_p[-1] for data_p in data_1]) / scale

data_2 = extract_component(data_d, "S", 3, 2)
...
fig.savefig(f'/scratch/cucui/GPUTensorProductSmoothers/build_DG/Figures/Smoothing_3D.png', dpi=fig.dpi)
fig.savefig(f'/scratch/cucui/GPUTensorProductSmoothers/build_DG/Figures/Smoothing_3D.pdf', dpi=fig.dpi)