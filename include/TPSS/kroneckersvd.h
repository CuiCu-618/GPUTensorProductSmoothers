...
namespace intern
{
   ...
   * @p singular_values. Left singular vectors are juxtaposed column-by-column
   * in
                         ...
                         Table<2, Number>              &VT)
  {
    ...
    const bool svd_data_fields_are_empty =
      U.empty() && singular_values.empty() && VT.empty();
    ...
    for (auto lane = 0U; lane < get_macro_size<Number>(); ++lane)
      {
        ...
        /// TODO compute only the singular_values.size() first singular values
        /// and not the complete SVD
  ...
  compute_bidiagonal_svd(const AlignedVector<Number> &diagonal,
                         ...
                         Table<2, Number>            &VT)
  {
    const ArrayView<const Number> diagonal_view(diagonal.data(),
                                                diagonal.size());
    const ArrayView<const Number> super_diagonal_view(super_diagonal.data(),
                                                      super_diagonal.size());
    compute_bidiagonal_svd<Number>(
      diagonal_view, super_diagonal_view, U, singular_values, VT);
  }
...
compute_ksvd_impl(
  ...
  const std::size_t lanczos_iterations_in = static_cast<std::size_t>(-1),
  const std::size_t max_tensor_rank_guess = static_cast<std::size_t>(-1))
{
  ...
  const std::size_t lanczos_iterations =
    lanczos_iterations_in == static_cast<std::size_t>(-1) ?
      std::min<std::size_t>(
        /*Simon: demanded_ksvd_rank * demanded_ksvd_rank + 10*/
        demanded_ksvd_rank + 10,
        max_matrix_rank_shuffled) :
      lanczos_iterations_in;

  AssertIndexRange(lanczos_iterations, max_matrix_rank_shuffled + 1);

  std::vector<AlignedVector<Number>> p = {
    AlignedVector<Number>(shuffled_matrix.n())};
  p.back()[0] = Number(1.);
  ...
  std::vector<AlignedVector<Number>> u = {
    AlignedVector<Number>(shuffled_matrix.m())};

  Assert(has_nearly_zero_abs(LinAlg::euclidean_norm(p.back()) - (Number)1.),
         ExcMessage("Euclidean norm of p_0 must be one!"));
  for (std::size_t i = 0;
       i <
       lanczos_iterations /*&& !has_nearly_zero_abs(std::abs(beta.back()))*/;
       ++i)
    {
      /// v_i = p_i / beta_i
      v.push_back(LinAlg::inverse_scaling_if(p.back(), beta.back()));
      // std::cout << "||v_" << i << "||: " <<
      // varray_to_string(LinAlg::euclidean_norm(v.back()))
      //           << std::endl;
      ...
      // std::cout << "||r_" << i << "||: " <<
      // varray_to_string(LinAlg::euclidean_norm(r.back()))
      //           << std::endl;
      ...
      // std::cout << "alpha_" << i << ": " << varray_to_string(alpha.back()) <<
      // std::endl;

      /// u_{i+1} = r_i / alpha_i
      u.push_back(LinAlg::inverse_scaling_if(r.back(), alpha.back()));
      // std::cout << "||u_" << i + 1 << "||: " <<
      // varray_to_string(LinAlg::euclidean_norm(u.back()))
      //           << std::endl;
      ...
      // std::cout << "beta_" << i + 1 << ": " << varray_to_string(beta.back())
      // << std::endl;
  ...
  /// discard the first value of beta (initial guess) and the last value (nearly
  /// zero)
  ...
  intern::compute_bidiagonal_svd(
    alpha_view, beta_view, tildeU, singular_values, tildeVT);
  ...
  const auto actual_ksvd_rank =
    std::min<std::size_t>(demanded_ksvd_rank, singular_values.size());
  ...
  for (std::size_t i = 0; i < actual_ksvd_rank; ++i)
    {
      ...
      A_i = LinAlg::sfolding_impl(left_singular_vectors,
                                  A_i.size(0),
                                  A_i.size(1),
                                  std::sqrt(singular_values[i]),
                                  i);
      B_i = LinAlg::sfoldingT_impl(right_singular_vectorsT,
                                   B_i.size(0),
                                   B_i.size(1),
                                   std::sqrt(singular_values[i]),
                                   i);
             ...
             const std::size_t lanczos_iterations_in = -1)
{
  ...
  Tensors::TensorProductMatrix<2, Number> shuffled_matrix(
    rank1_tensors_shuffled);
             ...
             const std::size_t lanczos_iterations_in = -1)
{
  ...
  shuffled_matrix.shuffle({A_0.size(0), A_0.size(1)},
                          {B_0.size(0), B_0.size(1)});

  return compute_ksvd_impl<Number, MatrixAsTable<Number>>(
    dst, shuffled_matrix, lanczos_iterations_in);
}
             ...
             const std::size_t lanczos_iterations_in = -1)
{
  ...
  return compute_ksvd_impl<Number, DiagonalMatrixWrap<Number>>(
    dst, shuffled_matrix, lanczos_iterations_in);
}
  ...
  for (std::size_t i = 0; i < in_rank; i++)
    {
      ...
      std::array<AlignedVector<Number>, 3> matrix_vect = {
        vectorize_matrix(in[i][2]),
