...
namespace Tensors
{
  ...
  make_zero_tensor(const std::array<IntType, order> rows,
  ...
  template <int order, typename Number, typename IntType = std::size_t>
  std::array<Table<2, Number>, order>
  make_id_tensor(const std::array<IntType, order> rows,
                 const std::array<IntType, order> columns)
  {
    std::array<Table<2, Number>, order> tensor;
    for (auto d = 0U; d < order; ++d)
  ...
  template <typename MatrixTypeIn1,
            ...
            typename MatrixTypeOut = MatrixTypeIn1>
  typename std::remove_const<
    typename std::remove_reference<MatrixTypeOut>::type>::type
  kronecker_product(MatrixTypeIn1 &&left_matrix, MatrixTypeIn2 &&right_matrix)
  {
    ...
    typename std::remove_const<
      typename std::remove_reference<MatrixTypeOut>::type>::type matrix_out;
  ...
  kronecker_product(
    const std::array<AlignedVector<Number>, order> &diagonals_1d)
  {
    ...
    for (unsigned int j = 0; j < matrix.n(); ++j)
      {
        ...
        const auto e_j_view =
          make_array_view<const Number>(e_j.begin(), e_j.end());
        const auto col_j_view =
          make_array_view<Number>(col_j.begin(), col_j.end());
  ...
  /**
   * Converts the inverse of a matrix into a two dimensional table. MatrixType
   * has to fulfill following interface:
   *
   * method m() returning number of rows
   * method n() returning number of cols
   * typedef value_type
  ...
  {
    std::vector<std::array<Table<2, Number>, order>> prod_of_tensors(
      ...
      for (auto i1 = 0U; i1 < tensors1.size(); ++i1)
        std::transform(
          ...
          [](const auto &A, const auto &B) {
            ...
            auto C = LinAlg::
              product_impl<Number, Number, false, transpose1, transpose2>(A, B);
  ...
  {
    std::vector<std::array<Table<2, Number>, order>> scaled_tensors(
      tensors.size());
    ...
    const std::array<Table<2, Number>, dim> &derivative_matrix)
  {
    ...
    if (dim == 1)
    ...
    else if (dim == 3)
      {
        ...
        auto &&AxMxM =
          kronecker_product(derivative_matrix[2],
                            kronecker_product(mass_matrix[1], mass_matrix[0]));
