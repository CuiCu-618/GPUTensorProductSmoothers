...
namespace Tensors
{
  ...
  TensorProductMatrixBase<order, Number, n_rows_1d>::operator=(
    const TensorProductMatrixBase &other)
  {
  ...
  TensorProductMatrixBase<order, Number, n_rows_1d>::reinit(
    const std::vector<
      typename TensorProductMatrixBase<order, Number, n_rows_1d>::tensor_type>
      &elementary_tensors_in)
  {
    ...
    tensor_helper_row =
      std::make_shared<const TensorHelper<order>>(n_rows_foreach_dimension);
    tensor_helper_column =
      std::make_shared<const TensorHelper<order>>(n_columns_foreach_dimension);

    Assert(
      check_n_rows_1d_static(elementary_tensors_in),
      ExcMessage(
        "Not all univariate matrices are of size (n_rows_1d x n_rows_1d)."));
    ...
    const ArrayView<const Number> &src_view) const
  {
    ...
    Assert(tensor_helper_row,
           ExcMessage("Did you initialize tensor_helper_row?"));
    Assert(tensor_helper_column,
           ExcMessage("Did you initialize tensor_helper_column?"));

    constexpr bool is_dynamic_path = n_rows_1d == -1;
    if (!is_dynamic_path)
      {
        Assert(
          tensor_helper_row->is_isotropic(),
          ExcMessage(
            "Currently, we support only isotropic tensors for the static path."));
        Assert(
          tensor_helper_column->is_isotropic(),
          ExcMessage(
            "Currently, we support only isotropic tensors for the static path."));
      }

    /// The evaluator is aware of (possible) anisotropic tensor structure (for
    /// the dynamic path).
    evaluator_type eval(tensor_helper_row->size(),
                        tensor_helper_column->size());
    ...
    /// tensors occuring during sum factorization. The evaluator has to take
    /// care of not writing noise into (maybe to large) arrays.
    const unsigned int mm_dynamic =
      eval.template max_intermediate_size<transpose>();
  ...
  TensorProductMatrix<order, Number, n_rows_1d>::operator=(
    const TensorProductMatrix &other)
  {
    ...
    const AdditionalData &additional_data_in)
  {
    ...
    if (additional_data.state == State::basic)
    ...
    else if (additional_data.state == State::ranktwo)
      {
        ...
        internal::
          ComputeGeneralizedEigendecomposition<order, Number, n_rows_1d>{}(
            this->eigenvalues,
            eigenvector_tensor,
            elementary_tensors_in,
            additional_data_in.spd_mask);
    ...
    else if (additional_data.state == State::rankone)
      {
        ...
        ranktwo_tensors.emplace_back(
          make_id_tensor<order, Number, unsigned int>(this->tensor_m(),
                                                      this->tensor_n()));
  ...
  {
    Assert(additional_data.state == State::ranktwo ||
             additional_data.state == State::separable ||
    ...
    for (unsigned int i = 0; i < eigenvalues.size(); ++i)
      {
        ...
        for (auto r = 0U; r < 2U; ++r)
          {
            const bool is_one =
              r == static_cast<unsigned int>(additional_data.spd_mask[0]);
            lambda_r = is_one ? static_cast<Number>(1.) :
                                eigenvalues_foreach_dimension[0][ii[0]];
            for (auto d = 1; d < order; ++d)
              {
                const bool is_one =
                  r == static_cast<unsigned int>(additional_data.spd_mask[d]);
                lambda_r *= is_one ? static_cast<Number>(1.) :
                                     eigenvalues_foreach_dimension[d][ii[d]];
              }
    ...
    const ArrayView<const Number> &src_view) const
  {
    ...
    if (additional_data.state == State::basic)
      {
        Assert(!additional_data.force_positive_definite_inverse,
               ExcMessage("Not supported."));
    ...
    const ArrayView<const Number> &src_view) const
  {
    ...
    ArrayView<Number> tmp_view =
      make_array_view(this->tmp_array.begin(), this->tmp_array.end());
    ...
    //                  eigenvalues.end(),
    //                  eigenvalues.begin(),
    //                  [](const auto & lambda) { return
    //                  zero_out_negative_value(lambda); });
