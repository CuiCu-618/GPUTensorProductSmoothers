...
namespace dealii
{
  namespace VectorToolsFix
  {
    ...
    namespace internals
    {
        ...
        AffineConstraints<double>                  &constraints)
      {
        ...
        const unsigned int
          unit_tangential_direction[GeometryInfo<2>::faces_per_cell] = {1,
                                                                        1,
                                                                        0,
                                                                        0};

        std::vector<Vector<double>> solution_values(
          fe_values.n_quadrature_points, Vector<double>(2));
        ...
        for (unsigned int q = 0; q < fe_values.n_quadrature_points; ++q)
          {
            ...
            /// ||DF(x_q) * t||_2   where t is the unit tangential at unit
            /// hyperface
            ...
            for (unsigned int i = 0; i < fe.dofs_per_face; ++i)
              {
                ...
                /// phi_i * n
                const double &phi_i_times_n =
                  (normals[q] * fe_values[vec].value(ii, q));
                dof_values(i) +=
                  u_times_n * phi_i_times_n * norm_of_DFx_times_t * dx;
              }
          }
        ...
        for (unsigned int i = 0; i < fe.dofs_per_face; ++i)
          {
            ...
            const bool is_not_constrained =
              !(constraints.is_constrained(global_dof_index));
            /// NOTE FE_RaviartThomas returns true for all vector components
            /// regardless of the hyperface, thus it suffices to query the first
            /// vector component
            const bool has_nonzero_component =
              fe.get_nonzero_components(ii)[first_vector_component];
            if (is_not_constrained && has_nonzero_component)
              {
                constraints.add_line(global_dof_index);
                if (std::abs(dof_values(i)) > 1e-14)
                  constraints.set_inhomogeneity(global_dof_index,
                                                dof_values(i));
              }
          }
      }
      ...
      const Mapping<dim>        &mapping)
    {
      ...
      /// the face integrals computed in
      /// compute_face_projection_div_conforming() are one-to-one to the
      /// Raviart-Thomas node functionals associated with hyperfaces: thus it
      /// suffices to constrain only those shape functions associated with a
      /// given hyperface

      const auto n_q_points_1d =
        dof_handler.get_fe().degree + 2; // !!! fe.degree+1
      QGauss<dim - 1> face_quadrature(n_q_points_1d);

      const auto update_flags_face = update_JxW_values | update_normal_vectors |
      ...
      /// project (dim-1)-dimensional quadrature points to dim-dimensional
      /// points for each hyperface
                        ...
                        if (dynamic_cast<const FESystem<dim> *>(
                              &cell->get_fe()) == nullptr)
                          {
                            ...
                            const bool is_raviartthomas_new =
                              dynamic_cast<const FE_RaviartThomas_new<dim> *>(
                                &cell->get_fe()) != nullptr;
                            const bool is_raviartthomasnodal_new =
                              dynamic_cast<
                                const FE_RaviartThomasNodal_new<dim> *>(
                                &cell->get_fe()) != nullptr;
                            AssertThrow(
                              is_raviartthomas || is_raviartthomas_new ||
                                is_raviartthomasnodal_new,
                              typename FiniteElement<
                                dim>::ExcInterpolationNotImplemented());
                          }

                        AssertDimension(cell->active_fe_index(),
                                        0); // hp-case has spec.
    ...
    // template<int dim>
    // void
    // project_boundary_values_div_conforming(const hp::DoFHandler<dim> &
    // dof_handler,
    //                                        const unsigned int
    //                                        first_vector_component, const
    //                                        Function<dim> & boundary_function,
    //                                        const types::boundary_id
    //                                        boundary_component,
    //                                        AffineConstraints<double> &
    //                                        constraints, const
    //                                        hp::MappingCollection<dim, dim> &
    //                                        mapping_collection)
    // {
    //   const unsigned int            spacedim      = dim;
    //   const hp::FECollection<dim> & fe_collection =
    //   dof_handler.get_fe_collection(); hp::QCollection<dim - 1>
    //   face_quadrature_collection; hp::QCollection<dim> quadrature_collection;
    ...
    //               nullptr)
    //               {
    //                 AssertThrow(dynamic_cast<const FE_RaviartThomas<dim>
    //                 *>(&cell->get_fe()) != nullptr,
    //                             typename
  ...
  namespace internal
  {
    namespace My
    {
       ...
       * requires the mode sizes of previous and subsequent dimensions.
       * Fortunately, the d-mode product is reduced to a 1-mode product of
       * order-3 tensors, where the new 0th mode is obtained by collapsing all
       * previous modes to dimension
       * @p direction and the new 2nd mode by collapsing all subsequent modes.
       * The
                            ...
                            const int n_post_dynamic                       = -1)
      {
        ...
        constexpr bool is_dynamic_mode =
          n_rows == -1 && n_columns == -1 && n_pre == -1 && n_post == -1;

        if (is_dynamic_mode) /// TODO static if-else
          {
            Assert(n_rows_dynamic > 0,
                   ExcMessage(
                     "Any reasonable n_rows_dynamic is at least one."));
            Assert(n_columns_dynamic > 0,
                   ExcMessage(
                     "Any reasonable n_columns_dynamic is at least one."));
            Assert(n_pre_dynamic > 0,
                   ExcMessage("Any reasonable n_pre_dynamic is at least one."));
            Assert(n_post_dynamic > 0,
                   ExcMessage(
                     "Any reasonable n_post_dynamic is at least one."));
          }
        else
          {
            /// TODO static_asserts if static if-else is used
            Assert(n_rows > 0,
                   ExcMessage("Any reasonable n_rows is at least one."));
            Assert(n_columns > 0,
                   ExcMessage("Any reasonable n_columns is at least one."));
            Assert(n_pre > 0,
                   ExcMessage("Any reasonable n_pre is at least one."));
            Assert(n_post > 0,
                   ExcMessage("Any reasonable n_post is at least one."));
          }
        ...
        const int mm =
          is_dynamic_mode ?
            (contract_over_rows ? n_rows_dynamic : n_columns_dynamic) :
            mm_static;
        const int nn =
          is_dynamic_mode ?
            (contract_over_rows ? n_columns_dynamic : n_rows_dynamic) :
            nn_static;
        const int n_columns_actual =
          is_dynamic_mode ? n_columns_dynamic : n_columns;
        ...
        Assert(n_columns_actual > 0,
               ExcMessage("Any reasonable n_columns_actual is at least one."));
        Assert(
          mm <= 128,
          ExcMessage(
            "For dynamic mm intermediate array x is initialized with static size 128."));
                            ...
                            const int                       n_post_dynamic)
      {
        ...
        Assert(n_rows_dynamic > 0,
               ExcMessage("Any reasonable n_rows_dynamic is at least one."));
        Assert(n_columns_dynamic > 0,
               ExcMessage("Any reasonable n_columns_dynamic is at least one."));
        Assert(n_pre_dynamic > 0,
               ExcMessage("Any reasonable n_pre_dynamic is at least one."));
        Assert(n_post_dynamic > 0,
               ExcMessage("Any reasonable n_post_dynamic is at least one."));
      ...
      /**
       * Tensor product evaluator with static loop bounds (assuming isotropic
       * tensors by default).
       */
      template <int dim,
                typename Number,
      ...
      {
        EvaluatorTensorProduct(const int /*dummy*/ = -1,
                               const int /*dummy*/ = -1){};
              ...
              const int /*n_post_dynamic*/    = -1)
        {
          ...
          constexpr int n_pre_actual =
            is_isotropic_path ? Utilities::pow(n_columns_actual, direction) :
                                n_pre;
          constexpr int n_post_actual =
            is_isotropic_path ?
              Utilities::pow(n_rows_actual, reverse_direction) :
              n_post;
      ...
      struct EvaluatorTensorProduct<dim, Number, -1, -1, Number2>
      {
        static_assert(1 <= dim && dim <= 3,
                      "This struct is implemented for up to 3 dimensions.");

        EvaluatorTensorProduct(const int n_rows_in, const int n_columns_in)
          : n_rows(n_rows_in)
          , n_columns(n_columns_in)
        {
          Assert(n_rows > 0,
                 ExcMessage("Any reasonable n_rows is at least one."));
          Assert(n_columns > 0,
              ...
              const int                       n_columns_in = -1) const
        {
          ...
          const int n_columns_actual =
            n_columns_in == -1 ? n_columns : n_columns_in;
          ...
          // const int n_pre_actual = collapse_sizes_pre(direction, size0_src,
          // size1_src, size2_src); const int n_post_actual =
          // collapse_sizes_post(direction, size0_dst, size1_dst, size2_dst);
