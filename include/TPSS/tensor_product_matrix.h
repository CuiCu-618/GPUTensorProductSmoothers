...
namespace Tensors
{
  namespace internal
  {
     ...
     * Kronecker factor. Furthermore, it facilitates static or dynamic loop
     * bounds.
     *
    ...
    struct Evaluator
      : public dealii::internal::My::
          EvaluatorTensorProduct<order, Number, n_rows_1d, n_rows_1d>
    {
      using Base = dealii::internal::My::
        EvaluatorTensorProduct<order, Number, n_rows_1d, n_rows_1d>;

      Evaluator(
        const std::array<unsigned int, order> &n_rows_foreach_dimension,
       ...
       * TensorProductEvaluator. Furthermore, the number of rows and columns of
       * the
      ...
      /**
       * Returns the maximum number of elements intermediate tensors have when a
      ...
      max_intermediate_size() const
      {
        ...
        for (auto d = 0; d < order; ++d)
          {
            const auto [n_pre, n_post, n_rows, n_columns] =
              anisotropic_indices(d);
            const unsigned int n_elements_of_direction =
              transpose ? n_columns : n_rows;
            const unsigned int size_tmp =
              n_pre * n_elements_of_direction * n_post;
        ...
        for (auto dimension = 0; dimension < order; ++dimension)
          {
            ...
            const auto &A =
              rank1_tensors[static_cast<std::size_t>(!B_index)][dimension];
            const auto &B =
              rank1_tensors[static_cast<std::size_t>(B_index)][dimension];
    ...
    /// TODO static path using template parameter n_rows_1d
    template <int order, typename Number, int n_rows_1d>
    struct ComputeGeneralizedEigendecomposition<order,
                                                ...
                                                n_rows_1d>
    {
      ...
      operator()(
        ...
        const std::vector<std::array<Table<2, VectorizedArray<Number>>, order>>
                                &rank1_tensors,
        const std::bitset<order> B_mask)
      {
        AssertDimension(rank1_tensors.size(), 2U);
        ...
        for (unsigned int d = 0; d < order; ++d)
          n_rows_max_dynamic =
            std::max<unsigned int>(n_rows_max_dynamic,
                                   rank1_tensors.front()[d].n_rows());
        const unsigned int n_flat_max = n_rows_1d > 0 ?
                                          n_rows_1d * macro_size :
                                          n_rows_max_dynamic * macro_size;
        const unsigned int nm_flat_max =
          n_rows_1d > 0 ? n_rows_1d * n_rows_1d * macro_size :
        ...
        for (auto dimension = 0; dimension < order; ++dimension)
          {
            ...
            vectorized_transpose_and_store(
              false, nm, &(B(0, 0)), offsets_nm.data(), B_flat.data());
            vectorized_transpose_and_store(
              false, nm, &(A(0, 0)), offsets_nm.data(), A_flat.data());
            ...
            vectorized_load_and_transpose(n,
                                          lambdas_flat.data(),
                                          offsets_n.data(),
                                          lambdas.begin());
   ...
   * complexity, for example used in a matrix-vector multiplication. If the
   * tensor structure qualifies for a more specific group of tensor product
   * matrices, one might use this specific State upon initialization (resulting
   * in even more efficient memory compression and/or application routines).
   *
   ...
   *
   * Some functionalities are supported only for a specific State but more
   * details are provided in the documentation there.
   */
  template <int order, typename Number, int n_rows_1d = -1>
  ...
  public:
    ...
    void
    vmult_add(const ArrayView<Number>       &dst_view,
              const ArrayView<const Number> &src_view) const;

    void
    Tvmult(const ArrayView<Number>       &dst_view,
           const ArrayView<const Number> &src_view) const;

    void
    Tvmult_add(const ArrayView<Number>       &dst_view,
               const ArrayView<const Number> &src_view) const;
    ...
    get_matrix_1d(const unsigned int tensor_index,
  ...
  protected:
     ...
     * "one-dimensional" matrices. The sum of these tensors determines the
     * matrix this class represents. Given all rank-1 tensors are linear
     * independent, the vector length denotes the tensor rank (or Kronecker
     * rank).
     *
  ...
  class TensorProductMatrix
    : public TensorProductMatrixBase<order, Number, n_rows_1d>
  {
  public:
    enum class State
    {
    ...
    struct AdditionalData
    {
       ...
       * symmetric, positive definite and on the right-hand side of the
    ...
    TensorProductMatrix(const std::vector<tensor_type> &elementary_tensors,
                        const AdditionalData           &additional_data =
                          {State::basic, std::bitset<order>{}, false});
     ...
     */
    TensorProductMatrix(
      const std::vector<tensor_type> &elementary_tensors,
     ...
     * matrix.
     *
     * basic : All rank-1 tensors are treated in the way they are passed, see
     * the class' description.
     *
     * ranktwo : Accepts two and only two rank-1 tensors passed by @p
     * elementary_tensors. During initialization a generalized
     * eigendecomposition is computed which enables an efficient implementation
     * of apply_inverse()
     ...
     * elementary_tensors. During initialization a generalized
     * eigendecomposition is computed which enables an efficient implementation
     * of apply_inverse() (see the class' description). The first rank-1 tensor
     ...
     * elementary_tensors. During initialization a specific eigendecomposition
    ...
    reinit(const std::vector<tensor_type> &elementary_tensors_in,
           const AdditionalData           &additional_data_in = {State::basic,
                                                                 std::bitset<order>{},
                                                                 false});
     ...
     *
     * basic : We do not compute the inverse during (re-)initialization such
     * that
     * non-invertible matrices are possible. The first time @p apply_inverse() is
     * called after (re-)initialization the inverse is computed (thus requiring
     * @p basic_inverse to be mutable).
     *
     ...
     * (re-)initialization to apply the inverse via fast diagonalization.
     *
     * rankone : We use the eigendecomposition computed during
     * (re-)initialization to apply the inverse via fast diagonalization.
     *
     * Note that for each vectorization lane of arithmetic type @p Number the
     * inverse of (nearly) zero eigenvalues is replaced by zero avoiding
     * division by zero.
     */
    void
     ...
     */
    void
    vmult_add(AlignedVector<Number>       &dst,
     ...
     */
    void
    Tvmult_add(AlignedVector<Number>       &dst,
               const AlignedVector<Number> &src) const;

    /// TODO does this conversion make sense?!...
    operator std::vector<
      std::vector<std::array<Table<2, Number>, order>>> const &() const
    {
      return this->elementary_tensors;
    }

    /**
     * Returns the number of rank-1 tensors used to initialize this tensor
     * product matrix. Depending on the linear independence of those tensors the
     * actual tensor rank is less than or equal to the maximal rank returned.
     */
    unsigned int
    n_max_rank() const;

    /**
     * Returns the eigenvalues of the underlying matrix, to be precise for the
     * @p
    ...
    /**
     * Returns the tensor of vectors containing the (generalized) eigenvalues
     * for each direction computed during reinit().
     *
     * This functionality is only supported in separable/ranktwo/rankone state.
     */
     ...
     * rank-1 tensor of univariate eigenvectors if the matrix is appropriate
     * (see the class' documentation on the requirements for a tensor product
     * matrix with valid ranktwo/separable/rankone state.
     *
     * This functionality is only supported in separable/ranktwo/rankone state.
     */
    Table<2, Number>
  ...
  private:
    ...
    /**
     * The (naively computed) inverse of the underlying matrix if in basic
     * state.
     */
     ...
     * times n() matrix can be computed as sum over Kronecker products of
     * diagonal matrices.
     */
    std::array<AlignedVector<Number>, order> eigenvalues;
  ...
  TensorProductMatrixBase<order, Number, n_rows_1d>::get_elementary_tensors()
    const
  {
  ...
  template <int order, typename Number, int n_rows_1d>
  inline const typename TensorProductMatrixBase<order, Number, n_rows_1d>::
    matrix_type_1d &
    TensorProductMatrixBase<order, Number, n_rows_1d>::get_matrix_1d(
  ...
  {
    Assert(tensor_helper_row,
           ExcMessage("tensor_helper_row is not initialized."));
  ...
  {
    Assert(tensor_helper_column,
           ExcMessage("tensor_helper_column is not initialized."));
  ...
  template <int order, typename Number, int n_rows_1d>
  inline unsigned int
  TensorProductMatrixBase<order, Number, n_rows_1d>::m(
  ...
  template <int order, typename Number, int n_rows_1d>
  inline unsigned int
  TensorProductMatrixBase<order, Number, n_rows_1d>::n(
    unsigned int dimension) const
  {
    AssertIndexRange(dimension, order);
    Assert(tensor_helper_column,
  ...
  TensorProductMatrixBase<order, Number, n_rows_1d>::
    check_n_rows_and_columns_1d_impl(
      ...
      const unsigned int                                      n_cols) const
  {
    return std::all_of(tensors.cbegin(),
                       ...
                       [direction, n_rows, n_cols](const auto &tensor) {
                         ...
                         return matrix.size(0) == n_rows &&
                                matrix.size(1) == n_cols;
                       });
  }
      ...
      const std::vector<std::array<Table<2, Number>, order>> &tensors) const
  {
    ...
    Assert(tensor_helper_column,
           ExcMessage("tensor_helper isn't initialized."));
    ...
    for (const auto tensor_of_matrices : tensors)
      {
        ...
        const bool has_equal_rows =
          *tensor_helper_row == n_rows_foreach_dimension;
        const bool has_equal_columns =
          *tensor_helper_column == n_columns_foreach_dimension;
    ...
    if (n_rows_1d > 0)
      {
        ...
        return std::all_of(list_of_dimensions.cbegin(),
                           list_of_dimensions.cend(),
                           [&](const auto &direction) {
                             return check_n_rows_and_columns_1d_impl(tensors,
                                                                     direction,
                                                                     n_rows_1d,
                                                                     n_rows_1d);
                           });
