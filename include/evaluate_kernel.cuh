...
namespace PSMF
{
  ...
  struct TPEvaluatorBase<T, n_dofs_1d, Number, LaplaceVariant::Basic, 2>
  {
          ...
          Number       *tmp)
    {
      static_cast<T *>(this)->vmult_impl(
        dst, src, mass_matrix, derivative_matrix, bilaplace_matrix, tmp);
    }
  ...
  //       :
  //       //                (row + g_row * 8) * n_dofs_1d + 2 * col + g_col *
  //       8;
  ...
  //       const unsigned int cd_idx =
  //         (dir == 0) ? (2 * col) * n_dofs_1d + row : (row)*n_dofs_1d + 2 *
  //         col;
  ...
  //               auto  c1 = (add || sub || cycle == 1) ? out[cd_idx + stride]
  //               : 0;
  ...
  //               wmma::load_matrix_sync(
  //                 b_frag,
  //                 &in[warpId * 8 * (8 + skew_double) + i * 4 * (8 +
  //                 skew_double)], 8 + skew_double);
  ...
  //                 {
  //                   const unsigned int shape_idx = row * n_dofs_1d_padding +
  //                   k; const unsigned int source_idx =
  ...
  //                   wmma::load_matrix_sync(
  //                     b_frag,
  //                     &in[(z * 8 + warpId) * 16 * (16 + skew_double) + i *
  //                     8], 16 + skew_double);
  // #pragma unroll
  //                   for (int t = 0; t < b_frag.num_elements; t++)
  //                     {
                      ...
                      2>
  {
               ...
               Number       *tmp)
    {
      ...
      __syncthreads();
      apply<1, true, false, true>(&derivative_matrix[offset], tmp, dst);
      // __syncthreads();
      // apply<0, false>(&derivative_matrix[offset], src, tmp);
      // __syncthreads();
      // apply<1, true>(derivative_matrix, tmp, dst);
    }
  };
        ...
        2>
  {
               ...
               Number       *tmp)
    {
      ...
      apply<0, false>(derivative_matrix, src, tmp);
      __syncthreads();
      apply<1, false, true, true>(derivative_matrix, tmp, dst);
    }
  };
  ...
  struct TPEvaluatorSmootherInv<Number, n_dofs_1d, SmootherVariant::GLOBAL, 2>
  {
                  ...
                  Number       *tmp)
    {
      ...
      src[threadIdx.y * n_dofs_1d + threadIdx.x % n_dofs_1d] /=
        (1 + eigenvalues[n_dofs_1d + threadIdx.y] *
  ...
  struct TPEvaluatorSmootherInv<Number, n_dofs_1d, SmootherVariant::FUSED_L, 2>
  {
                  ...
                  Number       *tmp)
    {
      ...
      if (linear_tid < (n_dofs_1d - 2) * (n_dofs_1d - 2))
        src[row * n_dofs_1d + col] /=
          (1 + eigenvalues[n_dofs_1d + row] * eigenvalues[col]);
      __syncthreads();
      apply<0, false>(eigenvectors, src, tmp);
      __syncthreads();
      apply<1, false, true>(&eigenvectors[offset], tmp, dst);
