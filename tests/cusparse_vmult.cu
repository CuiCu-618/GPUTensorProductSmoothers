...
namespace Step47
{
    ...
    public:
               ...
               const unsigned int /*component*/ = 0) const override
      {
        ...
        if (dim == 2)
          ...
          {
            return_gradient[0] = PI * std::cos(PI * p[0]) *
                                 std::sin(PI * p[1]) * std::sin(PI * p[2]);
            return_gradient[1] = PI * std::cos(PI * p[1]) *
                                 std::sin(PI * p[0]) * std::sin(PI * p[2]);
            return_gradient[2] = PI * std::cos(PI * p[2]) *
  ...
  struct CopyData
  {
    ...
    CopyData &
    operator=(const CopyData &) = default;

    CopyData &
    operator=(CopyData &&) = default;
  ...
  template <int dim>
  void
  BiharmonicProblem<dim>::assemble_system()
  {
    ...
    std::cout << "Finished in : " << time.wall_time() << " s\n"
              << "Memory Usage: "
              << system_matrix.memory_consumption() / 1024 / 1024 << " Mb\n\n";
  }
  ...
  template <int dim>
  void
  BiharmonicProblem<dim>::cuda_vmult()
  {
    std::cout << "cuda_vmult()...\n";

    LinearAlgebra::CUDAWrappers::Vector<double> cuda_vector(system_rhs.size());
    LinearAlgebra::ReadWriteVector<double>      rw_vector(system_rhs.size());
    rw_vector.import(system_rhs, VectorOperation::insert);
    cuda_vector.import(rw_vector, VectorOperation::insert);
    auto tmp = cuda_vector;


    Utilities::CUDA::Handle handle;

    CUDAWrappers::SparseMatrix<double> cuda_sparse_matrix(handle,
                                                          system_matrix);
    ...
    for (unsigned int i = 0; i < N; ++i)
      {
        ...
        cuda_sparse_matrix.vmult(cuda_vector, tmp);
    ...
    std::cout << "cuSparse vmult(): " << std::scientific << std::setprecision(4)
              << best_time << " s\n"
